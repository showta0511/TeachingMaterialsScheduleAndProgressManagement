        ...
        <div class="container">
            <form action="{{route('schedule.store')}}" method="post">
                ...
                @for ($i=0; $i<=$learning_period; $i++) <input type="hidden" name="user_id[]" value="{{Auth::id()}}">
                    ...
                    <!-- 日程の計算処理をdateに代入 -->
                    <?php $date = $first_day = date('Y-m-d', strtotime($first_day . '+' . $i . 'day')); ?>
                    ...
                    @if($page+$daily_learning_page<=$last_page) <input id="last_page" class="last_page" name="last_page[]" size="10" type="number" value="{{$page+$daily_learning_page}}"><br>
                        @endif
